...
#include <stdio.h>
#include <errno.h>  // Para sem_trywait
...
// LIMPIAR_COLA - VERSIÓN CORREGIDA
void limpiar_cola(ColaSolicitudes* cola){
    ...
    // IMPORTANTE: En lugar de destruir y recrear los semáforos,
    // los reiniciamos a sus valores iniciales de forma segura
    
    // 1. Reiniciar semáforo 'lleno' a MAX_SOLICITUDES_DIA
    ...
    // 2. Reiniciar semáforo 'vacio' a 0
...
// liberar nodos, para control del dia
void liberar_nodos(SistemaEcoFlow* sistema){
    ...
    printf("[SISTEMA] Todos los nodos liberados\n");
}
...
void liberar_consumo(SistemaEcoFlow* sistema){
    ...
    printf("[SISTEMA] Consumo diario reiniciado\n");
}
...
void limpiar_sistema(SistemaEcoFlow* sistema) {
    ...
    // Destrucción de semáforos de cola
    ...
    printf("[SISTEMA] Recursos liberados correctamente\n");
}
